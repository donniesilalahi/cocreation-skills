...
1. Read the project's configured `<workspaceRoot>/skills/cotranslate/SKILL.md` — that is your full operating guide. If it is not available, read `skills/cotranslate/SKILL.md` from this repository.
2. Run the `cotranslate` loop on the task you are given.
3. Write your memory-bank record under the consumer project's resolved `<workspaceRoot>/skills/cotranslate/memory-bank/` (default: .agents; create it if missing) — never inside the plugin.

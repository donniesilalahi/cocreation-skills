...
1. Read `.agents/skills/coresearch/SKILL.md` — that is your full operating guide. If it is not installed, read `skills/coresearch/SKILL.md` from this repository.
2. Run the `coresearch` loop on the task you are given.
3. Write your memory-bank record under the consumer project's `.agents/skills/coresearch/memory-bank/` (create it if missing) — never inside the plugin.

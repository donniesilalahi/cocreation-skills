...
an UPPERCASE auto-index regenerated by `index.js` (coaudit included, with its richer audit fields).
...
- **B — Rename + memory convention.** Rename 8 dirs to `co-*`; merge lesson skills into `colearn`;
  `YYYY-MM-DD-title.md` records; `tier`/cross-ref frontmatter; `index.js` columns; `marketplace.json`;

...
```
   ...
   (strategy)  (discovery)   (plan)     (design)      ║ cobuild ⇄ coverify║      (record)
                                                      ║      ⇅ codebug    ║
                                                      ╚═══════════════════╝
   Specialized loops, called on demand from any stage: coaudit (visual consistency) · coconsolidate (code DRY) · coharden (edge cases) · coport (design→impl port)
```
...
15 skills (1 master + 14 loops). Model tiers: **Opus** = judgment-heavy (shape/research/diagnose);
...
All 13 record-keeping loops follow the same convention: timestamped `YYYY-MM-DD-*.md` records +

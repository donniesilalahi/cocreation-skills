...
4. If the skill needs recall/tracking, scaffold `skills/<name>/memory-bank/` following an existing skill (e.g. `coplan`): an UPPERCASE index file with a `columns:` frontmatter spec, a `_template.md`, and an `index.mjs` copied/adapted to regenerate the table. Otherwise skip.
